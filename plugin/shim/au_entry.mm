...
#pragma mark - AU v3 Registration

/*
 * AU v3 Standalone Component Pattern
 * ─────────────────────────────────────────────────────────
 *
 * For .component bundles (not App Extension):
 * 1. registerSubclass() tells the system this is AU v3 (not AU v2)
 * 2. factoryFunction in Info.plist points to the AudioUnit class itself
 * 3. Custom UI is provided via requestViewControllerWithCompletionHandler:
 *
 * This avoids AUAudioUnitV2Bridge wrapping because the system knows
 * it's dealing with a native AU v3 component.
 */

#pragma mark - AU v3 Registration (Required for standalone .component)

@implementation DAWBridgeAudioUnit (Registration)

+ (void)load {
    AudioComponentDescription desc = {
        .componentType = kAudioUnitType_Effect,
        .componentSubType = 'dbri',
        .componentManufacturer = 'Dncr',
        .componentFlags = 0,
        .componentFlagsMask = 0
    };

    [AUAudioUnit registerSubclass:[DAWBridgeAudioUnit class]
           asComponentDescription:desc
                             name:@"Dancer: DAW Bridge"
                          version:66560];  // 1.4.0

    NSLog(@"[DAW Bridge] AU v3 registered via +load");
}

@end

/*
 * AU v2 Compatible Factory Function
 *
 * For standalone .component bundles to be recognized by the system,
 * a C factory function with the correct signature is required.
 *
 * The function returns an AUAudioUnit instance which the system
 * will wrap in AUAudioUnitV2Bridge if accessed via AU v2 APIs.
 *
 * Signature: void* FactoryFunction(const AudioComponentDescription* desc)
 */
extern "C" __attribute__((visibility("default")))
void* DAWBridgeViewControllerFactory(const AudioComponentDescription* inDesc) {
    @autoreleasepool {
        NSLog(@"[DAW Bridge] Factory function called - creating AU v3 instance");

        AudioComponentDescription desc = *inDesc;
        NSError *error = nil;

        // Create the AU v3 instance directly
        DAWBridgeAudioUnit *au = [[DAWBridgeAudioUnit alloc]
            initWithComponentDescription:desc
                                 options:0
                                   error:&error];

        if (error) {
            NSLog(@"[DAW Bridge] Factory error: %@", error);
            return nil;
        }

        // Return retained instance - system will release when done
        return (__bridge_retained void*)au;
    }
}

#pragma mark - Custom UI Provider

@implementation DAWBridgeAudioUnit (CustomUI)

- (void)requestViewControllerWithCompletionHandler:(void (^)(AUViewControllerBase * _Nullable))completionHandler {
    dispatch_async(dispatch_get_main_queue(), ^{
        // Check if we have an associated ViewController from Principal Class pattern
        if (self.associatedViewController) {
            NSLog(@"[DAW Bridge] Returning associated ViewController");
            completionHandler(self.associatedViewController);
            return;
        }

        // Create a new ViewController
        Class vcClass = NSClassFromString(@"DAWBridgeViewController");
        if (vcClass) {
            AUViewControllerBase *vc = [[vcClass alloc] init];

            // Set bidirectional reference
            [vc performSelector:@selector(setAudioUnit:) withObject:self];
            self.associatedViewController = vc;

            NSLog(@"[DAW Bridge] Created new ViewController for custom UI");
            completionHandler(vc);
        } else {
            NSLog(@"[DAW Bridge] ERROR: DAWBridgeViewController class not found");
